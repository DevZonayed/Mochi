...
Report the comms sync health for this repo. Provider defaults to `whatsapp`; if `accountId` is omitted and only one account is linked, use it — otherwise ask which.

1. Call `comms_account_status({provider, accountId})` and report `connected` / `needs_login` / `logged_out`.
